...
namespace A014_StringToNumber
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.Write("1. int로 변환할 문자열을 입력하세요:________\b\b\b\b\b\b\b\b");
            input = Console.ReadLine();
            bool result = Int32.TryParse(input, out value);

            if (!result)
            {
                Console.WriteLine($"'{input}'는 int로 변환할 수 없습니다.\n");
            }
            else
            {
            ...
            Console.Write("2. double로 변환할 문자열을 입력하세요:________\b\b\b\b\b\b\b\b");
            input = Console.ReadLine();
            try
            {
